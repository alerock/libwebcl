...
WebCLPlatform.prototype.createContext = function() {
    ...
    try {
        ...
        DEBUG("Selected the CPU");
    } catch (e) {
        context = WebCL.createContextFromType([WebCL.CL_CONTEXT_PLATFORM,
                                               this.platform],
                                              WebCL.CL_DEVICE_TYPE_GPU);
        DEBUG("Selected the GPU");
    }
...
function WebCLKernel(context, kernel)
{
    this.ctx = context;
    this.kernel = kernel;
    this.dummyArrayBuffer = new Int32Array(64);
    this.FAILRET = this.ctx.allocateData(this.dummyArrayBuffer, 16);
}

WebCLKernel.prototype.setArgument = function(index, memobj) {
    DEBUG("kernel.setArgument: index =", index, "arg =", memobj);
    this.kernel.setKernelArg(index+1, memobj.memobj, WebCL.types.MEM);
};
...
WebCLWrapper.prototype.getPlatform = function() {
    ...
    DEBUG("Selected the following OpenCL Platform: ", name);
